...
%		\label{eq:mfcdot}
	...
	In this section, we consider the opposite case to the one discussed in the previous section. In this case, we will zero out all terms in which degree $F < 3 $. 
	
	In this case, the action of the conjugate oscillator on an expression of type ${a^+}^na^m$ has the form
	
	\begin{equation}
		\label{eq:GenStrongfield}
		\mathcal{L}^*(t){a^+}^na^m = -i\dfrac{\chi}{2}\left((c_1 + c_4){a^+}^na^{m + 1} + (c_2 + c_3){a^+}^{n + 1}a^m + mc_3{a^+}^na^{m - 1} + nc_4{a^+}^{n - 1}a^m\right),
	\end{equation}
	where
	\begin{align*}
	...
	The plots of the mean particle number, linear operators and quadratic operators for the parameters $\ga = 1, \la = 10^{-2}, F = 10^2, \DE = 1, \chi = 1$ are shown in Fig. \ref{fig:strongn}, \ref{fig:stronga}, \ref{fig:stronga2}.
	
	\begin{figure}[h!]
		\begin{center}
			\begin{minipage}[h!]{0.45\linewidth}
				\includegraphics[width=1\linewidth]{Strongn.JPG}
				\caption{Average number of particles in the strong field}
				\label{fig:strongn}
			\end{minipage}
			\hfil
			\begin{minipage}[h!]{0.45\linewidth}
				\includegraphics[width=1\linewidth]{Stronga.JPG}
				\caption{Mean of the linear operator in a strong field}
				\label{fig:stronga}
			\end{minipage}
			\hfil
			\begin{minipage}[h!]{0.45\linewidth}
				\includegraphics[width=1\linewidth]{Stronga2.JPG}
				\caption{Mean of the quadratic operator in a strong field}
				\label{fig:stronga2}
			\end{minipage}
		\end{center}
	\end{figure}
	
	It is immediately apparent that in this regime the number of particles reaches a very large value of the order of $10^9$. It is also worth noting the fact that although the dynamics of damping of the number of particles is visible, it has oscillations, the number of particles has maxima and minima in time.
	...
	The authors are grateful to A.Yu. Karasev, A.M. Savchenko, R. Singh, S.D. Mostovoy, and A.S. Trushechkin for discussion of the problems discussed in the paper.
	
	The work of Kh. Meretukov was supported by the Foundation for the Advancement of Theoretical Physics and Mathematics “BASIS” under the grant № 23-2-1-34-1.
